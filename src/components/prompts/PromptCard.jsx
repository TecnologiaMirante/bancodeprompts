...
import {
  Heart, Copy, CheckCheck, Star, Eye, Pencil, Trash2, Sparkles,
  ThumbsUp, Download,
} from "lucide-react";
...
export default function PromptCard({
  ...
  const handleFavorite = (e) => { e.preventDefault(); e.stopPropagation(); toggleFav(prompt.id); };
  const handleLike     = (e) => { e.preventDefault(); e.stopPropagation(); toggleLike(prompt.id); };
  ...
  const handleExport = (e) => {
    ...
    const categoryNames = resolvedCategories.map((c) => c.name).join(", ") || "—";
    const lines = [
  ...
  return (
    ...
    >
      <div
        ref={wrapperRef}
        className="h-full"
        style={{ transformOrigin: "center center", transition: "transform 0.07s ease-out", willChange: "transform" }}
      >
        <Link to={`/prompt/${prompt.id}`} className="block h-full">
          <div className="relative bg-card border border-border rounded-2xl p-5 h-full flex flex-col shadow-card hover:shadow-card-hover hover:border-primary/20 overflow-hidden transition-[box-shadow,border-color] duration-300">
            ...
            {/* Mouse-tracking glow */}
            <div ref={glowRef}  className="absolute inset-0 rounded-2xl pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            <div ref={shineRef} className="absolute inset-0 rounded-2xl pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

            {/* Top accent bar */}
            <div className="absolute top-0 left-0 right-0 h-[2px] overflow-hidden rounded-t-2xl">
              <div className="absolute inset-0" style={{ backgroundColor: accentColor, opacity: 0.55 }} />
              <div className="absolute inset-0 card-accent-shimmer" style={{ "--shimmer-color": "rgba(255,255,255,0.55)" }} />
            </div>

            {/* Featured star */}
            {prompt.is_featured && !prompt.is_mirante_ia && (
              <motion.div className="absolute top-3.5 right-3.5"
            ...
            {prompt.is_mirante_ia && (
              ...
              >
                <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-900/25 border border-amber-300/60 dark:border-amber-600/40 shadow-sm">
                  <img src={dark ? logoEscuro : logoClaro} alt="" className="h-2.5 w-auto object-contain" />
                  <Sparkles className="w-2 h-2 text-amber-500" />
                  <span className="text-[9px] font-bold text-amber-600 dark:text-amber-400 tracking-widest uppercase leading-none">Mirante IA</span>
                </div>
              </motion.div>
            )}
            ...
            {resolvedSectors.length > 0 && (
              <div className="flex items-center gap-1 mb-2.5 flex-wrap">
                {resolvedSectors.slice(0, 2).map((s) => (
                  <span key={s.id} className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-md bg-surface border border-border text-muted-foreground">
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/50 gap-2">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {/* Counters */}
                {(prompt.viewCount > 0 || prompt.copyCount > 0 || likesCount > 0) && (
                ...
                {/* Edit */}
                {onEdit && (
                  <motion.button onClick={handleEdit} whileTap={{ scale: 0.8 }} whileHover={{ scale: 1.15 }}
                ...
                {/* Delete */}
                {onDelete && (
                  <motion.button onClick={handleDelete} whileTap={{ scale: 0.8 }} whileHover={{ scale: 1.15 }}
                ...
                {/* Export */}
                <motion.button onClick={handleExport} whileTap={{ scale: 0.8 }} whileHover={{ scale: 1.15 }}
                ...
                <motion.button onClick={handleCopy} whileTap={{ scale: 0.8 }} whileHover={{ scale: 1.15 }}
                ...
                {/* Like */}
                <motion.button onClick={handleLike} whileTap={{ scale: 0.8 }} whileHover={{ scale: 1.15 }}
                  className={`cursor-pointer w-6 h-6 flex items-center justify-center rounded-lg transition-colors ${
                    liked
                ...
                {/* Favorite */}
                <motion.button onClick={handleFavorite} whileTap={{ scale: 0.8 }} whileHover={{ scale: 1.15 }}
                  className={`cursor-pointer w-6 h-6 flex items-center justify-center rounded-lg transition-colors ${
                    favorited
